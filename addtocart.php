...
//$connection=mysqli_connect('localhost','root','write your password here')
	...
	if (isset($_POST['pid'])) {
	  ...
	  if (!$code) {
	    $query = $conn->prepare('INSERT INTO cart (label,price,image,qty,total_price,) VALUES(?,?,?,?,?)');
	...
	if (isset($_POST['action']) && isset($_POST['action']) == 'order') {
	  ...
	  $stmt = $conn->prepare('INSERT INTO orders  (name,email,image,address,pmode,amount_paid)VALUES(?,?,?,?,?,?)');
	  $stmt->bind_param('ssssss',$name,$email,$image,$address,$pmode,$grand_total);
	  $stmt->execute();
